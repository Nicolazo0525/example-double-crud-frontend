...
if (searchText.value.length > 0) {
    ...
    category.value = getCategories(perPage.value, searchText)
}
if (searchText.value.length <= 0) {
...
const changePage = (page) => {
    ...
    perPage.value = page;
    //Renderizo datos de esa página por medio de la función buscarcategorys
    searchCategories();
...
const pages = computed(() => {
    //Creo array
    const pages = [];
    //Recorro el array metacategory.value.last_page
    for (let i = 1; i <= metaCategories.value.last_page; i++) {
        pages.push(i);
    }
    //renderizo páginas
    return pages;
...
const delayedSearch = () =>{
    ...
    typingTimeout.value = setTimeout(searchCategories, 500)
}

function searchCategories(){
    category.value = getCategories(perPage.value, searchText.value)
}
...
<template>
    <div class="space-y-4 w-11/12 mx-auto rounded-md shadow-md bg-white mt-12 p-6">
        <div class="flex justify-between flex-row">
            ...
            <header class="mb-3 text-2xl font-bold">Index Categories</header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2">
            ...
            <div class="flex justify-end space-x-3 flex-col md:flex-row">
                <router-link :to="{name: 'CategoryCreate'}" class="w-20 text-center items-center rounded-2xl border-b-4 border-b-blue-600 bg-blue-500 py-3 font-bold text-white hover:bg-blue-400 active:translate-y-[0.125rem] active:border-b-blue-400">Create</router-link>
                ...
                <tbody>
                    <template v-if="categories.data">
                        <template v-for="category in categories.data" :key="category.id">
                            <tr>
                                ...
                                <td class="w-2/5 px-3 py-3  text-gray-600 text-base bg-white border-b border-gray-200">
                                    <div class="flex justify-center items-center">
                                            <img :src="`http://localhost:8000/storage/${category.image}`" class="w-10 h-10 rounded object-cover" alt="">
                                    </div>
                                </td>
                                <td class="w-2/5 px-3 py-3  text-gray-600 text-base bg-white border-b border-gray-200">
                                    <div class="flex justify-center items-center">
                                ...
                                <td class="w-2/5 px-3 py-3  text-gray-600 text-base bg-white border-b border-gray-200">
                                    <div class="flex justify-center gap-1 flex-row items-center">
                                        <div class=" items-center">
                                            <router-link :to="{name: 'BooksIndex', params:{category_id:category.id}}" class="">
                                        ...
                                        <div class="items-center">
                                            <router-link :to="{name: 'CategoryEdit', params:{id:category.id}}" class="">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        ...
                                        <div class=" items-center"><!-- 
                                            <button class="w-20 text-center items-center rounded-2xl" @click="deleteCategory(category.id)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                                </svg>
                                                
                                            </button> -->
                                            <button class="text-center rounded-2xl" @click="deleteOption(category.id)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                                </svg>
                                            </button>
                    ...
                    <div class="flex justify-center py-4">
                        <ul class="inline-flex -space-x-px">
                            ...
                            <li class="" v-if="metaCategories.current_page < metaCategories.last_page">
                                <a class="cursor-pointer bg-white border border-gray-300 text-gray-500 hover:bg-gray-100 hover:text-gray-700 rounded-r-lg leading-tight py-2 px-3 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white" @click.prevent="changePage(metaCategories.current_page + 1)">
                                    Siguiente
                                </a>
                            </li>
                        </ul>
